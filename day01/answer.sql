...
\COPY day01.sample (value) FROM 'day01/sample.txt' WITH (FORMAT CSV, FORCE_NULL (value));
\COPY day01.inputs (value) FROM 'day01/input.txt' WITH (FORMAT CSV, FORCE_NULL (value));

-- Part one
SELECT SUM(value) AS top_calories
FROM (
    SELECT value, COUNT(*) FILTER (WHERE value IS NULL) OVER (ORDER BY id) AS grp FROM day01.inputs
) AS g
GROUP BY grp
ORDER BY top_calories DESC FETCH FIRST ROW ONLY;

-- Part two
WITH top_three_elves AS (
    SELECT SUM(value) AS elf_calories
    FROM (
        SELECT value, COUNT(*) FILTER (WHERE value IS NULL) OVER (ORDER BY id) AS grp FROM day01.inputs
    ) AS g
    GROUP BY grp
